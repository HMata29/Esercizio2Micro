...
public class PizzaServiceImpl implements PizzaService {
    ...
    @Override
    public List<Pizza> addPizzasToRestaurants(List<RestaurantsIds> restaurantsIds) {
        List <RestaurantsIds> result = restaurantIdsRepository.saveAll(restaurantsIds);  //prima salvo
        List <Pizza> pizzas = new ArrayList<>(result.size());
        for(RestaurantsIds el : result){
            pizzas.add(pizzaRepository.findById(el.getPizzaId()).get()); //.get è perche è un tipo Optional, quindi da dover "predere"
        }
        return pizzas;
    }
}
